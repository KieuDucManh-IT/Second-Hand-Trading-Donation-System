...
exports.payOrderByWallet = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Thanh toán bằng ví thành công. Tiền đang được hệ thống giữ.",
      order,
    });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: error.message || "Không thể thanh toán bằng ví",
    });
  }
};
...
exports.cancelOrderAndRefund = async (req, res) => {
  try {
    ...
    const order = await escrowService.cancelOrderAndRefund(
      orderId,
      userId,
      reason
    );
    ...
    res.json({
      success: true,
      message: "Đã hủy đơn hàng và hoàn tiền về ví",
      order,
    });
  } catch (error) {
    res.status(400).json({
...
exports.markOrderShipping = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Đã cập nhật đơn hàng sang trạng thái đang giao",
      order,
    });
  } catch (error) {
    res.status(400).json({
...
exports.markOrderDelivered = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Đã giao hàng. Hệ thống bắt đầu đếm 7 ngày xác nhận.",
      order,
    });
  } catch (error) {
    res.status(400).json({
      success: false,
      message: error.message || "Không thể cập nhật đã giao hàng",
    });
  }
};
...
exports.buyerConfirmReceived = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Đã xác nhận nhận hàng. Tiền đã được chuyển vào ví người bán.",
      order,
    });
  } catch (error) {
    res.status(400).json({
...
exports.openOrderDispute = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Đã mở khiếu nại. Hệ thống sẽ tạm dừng chuyển tiền.",
      order,
    });
  } catch (error) {
    res.status(400).json({
...
exports.manualRunAutoRelease = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "Đã chạy kiểm tra tự động chuyển tiền",
      result,
    });
  } catch (error) {
    res.status(500).json({
