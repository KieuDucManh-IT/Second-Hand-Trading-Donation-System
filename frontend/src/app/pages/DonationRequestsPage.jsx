...
export default function DonationRequestsPage() {
  ...
  const loadDonations = async () => {
    try {
      ...
      setDonations(
        Array.isArray(data) ? data : data.donations || data.data || [],
      );
    } catch (error) {
      console.error(error);
        ...
        <>
          {pagedDonations.map((item) => (
            <div key={item._id} className="border rounded-lg p-4 mb-4 shadow">
              ...
              <p>Người yêu cầu: {item.requesterId?.fullName}</p>

              <p>Người đăng: {item.donorId?.fullName}</p>

              <p>Tin nhắn: {item.message}</p>

              {item.shippingInfo &&
                (item.shippingInfo.name || item.shippingInfo.address) && (
              ...
              {item.status === "pending" && (
                <div className="flex gap-3 mt-3">
                  <button
                    onClick={() => acceptDonation(item._id)}
