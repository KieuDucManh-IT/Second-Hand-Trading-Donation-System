...
import { Star, ArrowLeft, Calendar, MessageSquare, Tag, Award } from 'lucide-react';
import { ImageWithFallback } from '../components/figma/ImageWithFallback';
import { fetchProductById, ApiProduct } from '../api/productApi';
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center">
        <div className="text-center flex flex-col items-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">Đang tải thông tin và đánh giá...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-4">
        <p className="text-red-500 font-semibold mb-4">{error || 'Không tìm thấy sản phẩm'}</p>
        <Button onClick={() => navigate(-1)} className="rounded-xl px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white font-semibold">
          Quay lại
        </Button>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <Badge variant="outline" className="text-xs px-2.5 py-0.5 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-350">
                      {product.categoryId?.name || 'Sản phẩm'}
                    </Badge>
                    <Badge className={`text-xs px-2.5 py-0.5 font-medium ${product.condition === 'new' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' :
                        product.condition === 'like_new' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                          product.condition === 'good' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' :
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="border border-slate-100 dark:border-slate-800/80 bg-white/90 dark:bg-slate-950/40 shadow-sm rounded-2xl flex flex-col justify-center items-center p-6 text-center">
            <h3 className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-2">Đánh giá trung bình</h3>
            ...
            <p className="text-xs text-slate-450 dark:text-slate-500">({totalReviews} đánh giá toàn bộ)</p>
          </Card>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <span className="text-sm font-semibold text-slate-500 mr-2">Lọc theo sao:</span>
          <Button
          ...
          <CardContent className="p-0 divide-y divide-slate-100 dark:divide-slate-800/60">
            {filteredReviews.length === 0 ? (
              <div className="p-12 text-center flex flex-col items-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-medium">Không tìm thấy đánh giá phù hợp</p>
                {selectedStarFilter && (
                  <Button
                    variant="link"
                    onClick={() => setSelectedStarFilter(null)}
              ...
              filteredReviews.map((review) => {
                ...
                const dateVal = review.sellerRating?.ratedAt ? new Date(review.sellerRating.ratedAt).toLocaleDateString('vi-VN', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                }) : '—';
                ...
                return (
                  <div key={review._id} className="p-6">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div
                        onClick={() => review.buyerId?._id && navigate(`/profile/${review.buyerId._id}`)}
                      ...
                      >
                        <Avatar className="w-10 h-10 border border-slate-100 transition-transform group-hover/buyer:scale-105">
                          ...
                          <AvatarFallback className="bg-green-150 text-green-700 text-sm font-bold">
                            {review.buyerId?.fullName?.[0]?.toUpperCase() || 'U'}
                          </AvatarFallback>
                        </Avatar>
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-xs text-slate-400">{dateVal}</span>
