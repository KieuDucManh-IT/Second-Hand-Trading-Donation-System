...
export function ConfigTab() {
  ...
  const handleAddKeyword = (e) => {
    ...
    if (invalidWords.length > 0) {
      toast.error(
        `Từ khóa quá ngắn (ít hơn 2 ký tự): ${invalidWords.join(", ")}`,
      );
    }
    ...
    <Card className="border-slate-200/80 bg-white/85 shadow-sm dark:border-slate-800/70 dark:bg-slate-950/40">
      <CardHeader className="flex flex-row items-center gap-4">
        ...
        <div>
          ...
          <CardDescription>
            Quản lý các thông số chung của toàn bộ hệ thống.
          </CardDescription>
        </div>
      </CardHeader>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-amber-800/90 dark:text-amber-400/95">
                Các từ khóa trong danh sách này sẽ được dùng để kiểm duyệt nội
                dung tự động khi người dùng đăng bán sản phẩm mới hoặc cập nhật
                thông tin sản phẩm. Bài viết chứa từ khóa này sẽ bị chặn đăng và
        ...
        <div className="space-y-3">
          ...
          {keywords.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 py-8 text-center text-sm text-slate-400 dark:border-slate-800">
              Chưa cấu hình từ khóa cấm nào. Hệ thống sẽ sử dụng danh sách từ
              khóa mặc định.
            </div>
          ) : (
