...
import { emptyData } from "./managerDashboardTypes";
...
const validateInput = (value, fieldName, isDescription = false) => {
  ...
  if (/(.)\1{3,}/i.test(trimmed)) {
    toast.error(
      `${fieldName} không được chứa một ký tự lặp lại quá 3 lần liên tiếp.`,
    );
    return false;
  }
...
export function useManagerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    if (isPageReload) {
      const savedTab = sessionStorage.getItem("manager_dashboard_active_tab");
      if (
        savedTab &&
        [
          "products",
  ...
  const refreshDashboard = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error(result.message || "Không thể tải dữ liệu bảng quản lý");
  ...
  const repairExchangeProducts = async () => {
    try {
      ...
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.message || "Lỗi khi sửa sản phẩm");
  ...
  const updateProductStatus = async (productId, status) => {
    try {
      ...
      toast.success(
        status === "available"
          ? "Sản phẩm hiện đã hiển thị"
          : "Sản phẩm hiện đã được ẩn",
      );
      await refreshDashboard();
    } catch (err) {
  ...
  const updateReportStatus = async (reportId, endpoint) => {
    let reason = "";
    if (endpoint === "accept") {
      const inputReason =
        window.prompt("Nhập lý do chấp nhận báo cáo và cảnh cáo người dùng:") ||
        "";
    ...
    try {
      const response = await fetch(
        `${API_URL}/reports/${reportId}/${endpoint}`,
        {
          method: "PATCH",
          headers: authHeaders(),
      ...
      toast.success(
        endpoint === "accept"
          ? "Đã chấp nhận báo cáo và cảnh cáo"
          : "Đã từ chối báo cáo",
      );
      await refreshDashboard();
    } catch (err) {
  ...
  const updateUserStatus = async (userId, status) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "Không thể cập nhật trạng thái người dùng",
        );
      }
  ...
  const productViewList = showAllProducts
    ? allProductsList
