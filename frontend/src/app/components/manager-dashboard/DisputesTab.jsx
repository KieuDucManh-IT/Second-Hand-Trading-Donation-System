...
function getDisplayName(user, fallback = "Người dùng") {
  ...
  return user.fullName || user.name || user.username || user.email || fallback;
}
...
function getReporterEvidences(dispute) {
  return dispute?.evidences || dispute?.complaint?.evidences || [];
}

function getReportedDeliveryVideos(dispute) {
  if (!dispute) return [];
...
function getReportedPartyLabel(dispute) {
  return dispute?.otherParty || "Bên bị khiếu nại";
}

function MediaEvidenceGrid({ items, emptyText }) {
    ...
    <div className="grid grid-cols-2 gap-3">
      {items.map((ev, idx) => {
        ...
        return (
          ...
          >
            {isVideoFile(ev) ||
            String(mediaUrl).match(/\.(mp4|mov|webm)(\?|$)/i) ? (
...
export function DisputesTab({ disputesData, resolveDispute }) {
  ...
  const handleToggleHistory = (historyMode) => {
  ...
  const orderDisputes = (disputesData.orders || []).map((o) => ({
    ...
    complaintReason:
      o.complaint?.reason || o.disputeReason || "Khiếu nại giao dịch",
  ...
  const exchangeDisputes = (disputesData.exchanges || []).map((e) => {
    ...
    return {
      ...
      complaintReason:
        e.complaint?.reason || e.disputeReason || "Khiếu nại giao dịch",
  ...
  const filteredDisputes = allDisputes.filter((dispute) => {
    ...
    if (
      selectedStatus !== "all" &&
      dispute.complaintStatus !== selectedStatus
    ) {
      return false;
    }
  ...
  return (
    <>
                    ...
                    <TableCell className="align-middle">
                      {dispute.type === "order" ? (
                        <button
                          ...
                          title={
                            dispute.productId?.title || "Xem chi tiết sản phẩm"
                          }
                        >
                        ...
                        <div className="flex flex-col gap-1.5">
                          <button
                            ...
                            title={
                              dispute.requesterProduct?.title ||
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && handleCloseResolve()}
      >
        {selectedDispute && (
          <DialogContent className="max-w-2xl rounded-3xl border-slate-200 bg-white/95 p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-950/95 max-h-[90vh] overflow-y-auto">
            <DialogHeader className="text-left">
              ...
              <DialogDescription className="text-slate-500 dark:text-slate-400 text-xs">
                Vui lòng xem kỹ bằng chứng, video giao/đóng hàng và đưa ra hướng
                xử lý công bằng.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/20 p-4 space-y-3">
                ...
                <div className="space-y-4">
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 space-y-2">
                    ...
                    <div className="mt-2">
                      <span className="text-slate-400 block text-xs mb-1.5 font-medium">
                        Bằng chứng của người gửi báo cáo (
                        {getReporterEvidences(selectedDispute).length || 0})
                      </span>
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 space-y-2">
                    <span className="text-slate-800 dark:text-slate-200 font-semibold text-xs block uppercase tracking-wider">
                      Video đóng/giao hàng của bên bị báo cáo (
                      {getReportedPartyLabel(selectedDispute)})
                    </span>
                    ...
                    <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 space-y-2">
                      ...
                      <div className="mt-2">
                        <span className="text-slate-400 block text-xs mb-1.5 font-medium">
                          ...
                          {selectedDispute.counterComplaint.evidences?.length ||
                            0}
                          )
                        </span>
                        ...
                        <MediaEvidenceGrid
                          items={
                            selectedDispute.counterComplaint.evidences || []
                          }
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 space-y-4">
                <div className="rounded-xl bg-indigo-500/10 p-3 border border-indigo-500/20 text-xs text-indigo-700 dark:text-indigo-300 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    Manager có thể hoàn toàn bộ giá trị cho bên A, hoàn toàn bộ
                    giá trị cho bên B, hoặc từ chối giải quyết để giao dịch tiếp
                    tục cơ chế tự động hoàn trong 7 ngày.
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        setResolutionType("continue_auto_release")
                      }
                      className="mt-1"
                    />
                    ...
                    <div>
                      ...
                      <span className="text-xs opacity-80">
                        Không phán quyết thắng/thua. Giao dịch quay lại cơ chế
                        tự động hoàn tiền theo thời hạn.
                      </span>
                    </div>
                  </label>
                </div>
