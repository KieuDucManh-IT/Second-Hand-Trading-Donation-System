 ...
 * Component chuông thông báo cho Header – hiển thị số badge chưa đọc
 * và dropdown preview 5 thông báo mới nhất.
 *
 * Dùng trong Header.tsx:
 *   import NotificationBell from "./NotificationBell";
 *   // Thay thế icon Bell cũ bằng: <NotificationBell />
 */
...
function getTypeColor(type: string) {
  ...
  if (type === "order_cancelled" || type === "order_disputed") return "bg-red-500";
...
export default function NotificationBell() {
  ...
  const handleClickItem = (noti: INotification) => {
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50 dark:border-gray-700/50">
            <span className="font-semibold text-gray-800 dark:text-gray-200">Thông báo</span>
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-gray-50 dark:divide-gray-700/50">
            {preview.length === 0 ? (
              <div className="py-8 text-center text-gray-400 dark:text-gray-500 text-sm">Chưa có thông báo nào</div>
            ) : (
              preview.map((noti) => (
                <div
                  key={noti._id}
                ...
                >
                  <div className={`mt-1 flex-shrink-0 w-2.5 h-2.5 rounded-full ${getTypeColor(noti.type)}`} />
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm truncate ${!noti.isRead ? "font-semibold text-gray-900 dark:text-white" : "text-gray-700 dark:text-gray-300"}`}>
                      {noti.title}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5">{noti.message}</p>
                    <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-0.5">{timeAgo(noti.createdAt)}</p>
                  </div>
                </div>
              ))
            )}
